...
st.markdown(
    ...
    .chip-row {display:flex;gap:8px;flex-wrap:wrap;margin-top:16px;margin-left:60px;}
    .chip {
        display:inline-flex;
        align-items:center;
        gap:6px;
    ...
    .dot {width:7px;height:7px;border-radius:50%;background:#34d399;box-shadow:0 0 10px #34d399;}
    ...
    div[data-testid="stChatMessage"] p {font-size:15px;line-height:1.55;}
    ...
    .status-label {font-size:12px;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:.06em;}
    .status-value {font-size:14px;color:#0f172a;font-weight:700;margin-top:3px;}
    .section-kicker {color:#6366f1;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;}
    .section-title {font-size:22px;font-weight:800;color:#0f172a;margin-bottom:2px;}
    .section-copy {font-size:13px;color:#64748b;margin-bottom:14px;}
...
st.markdown(
    ...
    <div class="agent-hero">
      <div class="agent-title"><div class="agent-orb">✦</div><span>DocPilot AI</span></div>
      <div class="agent-subtitle">Document Intelligence Agent · AI QA Student Testing Lab</div>
      <div class="chip-row">
        <span class="chip"><span class="dot"></span> Agent online</span>
        <span class="chip">⚡ {MODEL}</span>
        <span class="chip">📄 PDF grounded</span>
        <span class="chip">🧪 QA workspace</span>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
    ...
    with st.container(border=True):
        ...
        if uploaded and st.button("⚡ Process document", type="primary", use_container_width=True):
            ...
            if not chunks:
                ...
                st.success(f"Indexed {len(chunks)} searchable chunks")
...
if st.session_state.pdf_name:
    ...
    s2.markdown(f'<div class="status-card"><div class="status-label">Agent status</div><div class="status-value">🟢 Ready for questions</div></div>', unsafe_allow_html=True)
...
with tab_tests:
    st.markdown('<div class="section-kicker">QA execution</div><div class="section-title">Test case workspace</div>', unsafe_allow_html=True)
    st.caption(f"Target: {TARGET_TESTS} test cases across multiple AI QA categories.")
    with st.container(border=True):
        with st.form("test_form", clear_on_submit=True):
            ...
            status = c2.selectbox("Execution Status *", ["PASS", "FAIL"])
            question = c2.text_area("Prompt / Input *", value=st.session_state.last_question)
            expected = st.text_area("Expected Result *")
            actual = st.text_area("Actual Agent Result *", value=st.session_state.last_answer)
            notes = st.text_area("Tester Notes")
            save = st.form_submit_button("＋ Add test case", type="primary", use_container_width=True)
    if save:
    ...
    if st.session_state.tests:
        ...
        st.download_button("↓ Export test cases CSV", df.to_csv(index=False).encode(), f"{st.session_state.student_name or 'student'}_test_cases.csv", "text/csv")

with tab_bugs:
    st.markdown('<div class="section-kicker">Defect triage</div><div class="section-title">Bug reporting workspace</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        with st.form("bug_form", clear_on_submit=True):
            ...
            save_bug = st.form_submit_button("＋ Log defect", type="primary", use_container_width=True)
    if save_bug:
        if not st.session_state.student_name.strip():
            st.error("Enter Student Name first.")
    ...
    if st.session_state.bugs:
        ...
        st.download_button("↓ Export defects CSV", df.to_csv(index=False).encode(), f"{st.session_state.student_name or 'student'}_bugs.csv", "text/csv")

with tab_score:
    st.markdown('<div class="section-kicker">Evaluation</div><div class="section-title">QA performance dashboard</div><div class="section-copy">Your score reflects test design, execution evidence, defect quality, and linkage.</div>', unsafe_allow_html=True)
    ts = test_score()
    bs = bug_score()
    ...
    m1.metric("Questions", q_count)
    m2.metric("Test Cases", len(st.session_state.tests), f"Target {TARGET_TESTS}")
    m3.metric("Defects", len(st.session_state.bugs), f"Target {TARGET_BUGS}")
    m4.metric("Threshold", f"{PASS_THRESHOLD}%")
    c1, c2, c3 = st.columns(3)
    c1.metric("Test Design", f"{ts}%")
    c2.metric("Defect Quality", f"{bs}%")
    c3.metric("Overall QA Score", f"{overall}%")
    if result == "PASS":
    ...
    report = {
        ...
        "result": result,
        "test_cases": st.session_state.tests,
        "bugs": st.session_state.bugs,
    }
    st.download_button("↓ Download final QA report", json.dumps(report, indent=2).encode(), f"{st.session_state.student_name or 'student'}_final_report.json", "application/json", type="primary")
    st.caption("Session data is temporary. Download your files before closing or resetting the app.")
